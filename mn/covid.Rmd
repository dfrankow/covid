...
hospcapdata_used <- hospcapdata_wide %>%
  ...
  pivot_longer(cols=c('icu_bed_capacity_used',
                      'ventilator_capacity_used',
                      'non_icu_bed_capacity_used'),
               names_pattern = "(.+)_capacity_used",
...
hospcapdata_used <- hospcapdata_used %>%
  mutate(
    type = recode(type,
                  "icu_bed" = "ICU bed",
                  "non_icu_bed" = "non-ICU bed"),
    alpha = if_else(date >= (hospcapdata_maxdate - 7), 0.35, 1))

ggplot(hospcapdata_used, aes(date, 100*capacity_used,
                             group=type,
                             color=type,
                             alpha=alpha)) +
  ...
  labs(title="Bed and ventilator capacity used",
       subtitle="Not including surge capacity.  Data may change in last 7 days.",
       y="% capacity used") +
  scale_alpha_identity()
```
