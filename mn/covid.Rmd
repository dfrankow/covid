...
wastewater_graph <- function() {
  wwdata <- read_csv('data/Wastewater Treatment COVID-19 Monitoring Dashboard.csv')
  names(wwdata) <- c(
    'date',
    'viral_load', 'viral_load_stderr',
    'total_cases', 'new_cases',
    'cases_per_capita', 'cases_per_capita_7day',
    # Don't know what these are
    'X8', 'X9'
  )
  wwdata <- add_rolling_num(wwdata, 'viral_load', ROLLING_WINDOW)
  viral_load_percent <- change_in_percent(wwdata$viral_load_rolling, CHANGE_WINDOW)$str

  p<-ggplot(wwdata %>% filter(date >= first_day),
         aes(date, viral_load_rolling)) +
    geom_line() +
    geom_point(aes(y=viral_load), size=1, alpha=0.3) +
    labs(y="viral load (avg), M copies/person/day",
         title='Wastewater viral load',
         subtitle=paste0('As of ', max(wwdata$date),
           ". Average viral load ",
                  round(tail(wwdata$viral_load_rolling, 1)),
                  ", 14-day change ", viral_load_percent)
         )
...
ggplot(casedataf %>% filter(date > first_day),
  ...
  geom_line(data=casedataf1 %>% filter(date >= first_day),
            aes(y=positive_cases_rolling), size=1) +
...
tests_plot <- function() {
  # test plot
  ggplot(labdataf %>% filter(date >= first_day), aes(date, daily_tests)) +
    geom_point(aes(alpha=alpha)) +
    geom_line(data=labdataf %>% filter(date >= first_day),
              aes(alpha=alpha)) +
    geom_line(data=labdataf1 %>% filter(date >= first_day),
              aes(y=daily_tests_rolling), size=1) +
