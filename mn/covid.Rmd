...
# last_day <- hospdata %>% filter(non_icu > 0) %>% pull(date) %>% max(na.rm=T)
...
hospdata <- add_rolling_num(hospdata, 'icu', 7)
hospdata <- add_rolling_num(hospdata, 'non_icu', 7)

# change in icu7 over last 7 days
icu7_delta7_percent <- change_in_percent(hospdata$icu7, CHANGE_WINDOW)

# change in non_icu7 over last 7 days
non_icu7_delta7_percent <- change_in_percent(hospdata$non_icu7, CHANGE_WINDOW)

hospdata_long <- hospdata %>%
  pivot_longer(c('icu', 'icu7', 'non_icu', 'non_icu7')) %>%
  filter(date >= first_day) %>%
  mutate(color = ifelse(name %in% c('icu', 'icu7'), 'icu', 'non_icu'),
         size = ifelse(name %in% c('icu', 'non_icu'), 0.3, 1))

ggplot(hospdata_long %>% filter(name %in% c('icu', 'non_icu')),
       aes(date, value, group=name, color=color)) +
  geom_point(alpha=0.3) +
  geom_line(data=hospdata_long %>% filter(name %in% c('icu7', 'non_icu7')),
            aes(size=size, alpha=size)) +
  geom_line(data=hospdata_long %>% filter(name %in% c('icu', 'non_icu')),
            aes(size=size, alpha=size)) +
  labs(title="Hospitalized",
       subtitle=paste0(
                "icu average ", round(tail(hospdata$icu7, 1)),
                ", change ", icu7_delta7_percent,
                ", non_icu average ",
                round(tail(hospdata$non_icu7, 1)),
                ", change ", non_icu7_delta7_percent),
...
ggplot(deathdata %>% filter(date >= first_day),
  ...
  geom_point(aes(y=daily_deaths), alpha=0.3) +
  geom_line(aes(y=daily_deaths7), size=1) +
  geom_line(aes(y=daily_deaths), alpha=0.3) +
...
the_alpha <- 0.1
casedata$alpha <- c(rep(0.3, nrow(casedata)-7), rep(the_alpha, 7))
...
ggplot(casedataf,
  ...
  labs(title="Daily confirmed cases",
       subtitle=paste0(
         ...
         ", change ", change_in_percent(casedataf1$daily_cases7, CHANGE_WINDOW), ".",
...
# make alpha of last 7 days less
labdata$alpha <- c(rep(0.3, nrow(labdata)-7), rep(the_alpha, 7))
...
ggplot(labdataf, aes(date, daily_tests)) +
  ...
  labs(title="Daily tests",
       subtitle=paste0(
         ...
         ", change ", change_in_percent(labdataf1$daily_tests7, CHANGE_WINDOW), ".",
         "  Last 7 days may have incomplete data.")) +
  scale_y_continuous(limits=c(0,NA)) +
  scale_alpha_identity()
...
caselabdata$alpha <- c(rep(1, nrow(caselabdata)-7), rep(0.3, 7))
