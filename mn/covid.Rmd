...
casedata <- casedata %>%
  ...
  add_rolling_num('positive_cases', 7)
...
ggplot(casedataf,
  ...
  geom_line(data=casedataf1, aes(y=positive_cases7), size=1) +
  labs(title="Daily confirmed cases (PCR+antigen)",
       subtitle=paste0(
         "average ", round(tail(casedataf1$positive_cases7, 1)),
         ", 14-day change ",
         change_in_percent(casedataf1$positive_cases7, CHANGE_WINDOW), ".",
         "  Last 7 days may have incomplete data.")) +
  scale_y_continuous(limits=c(0,NA)) +
  scale_alpha_identity() +
  scale_x_date(date_minor_breaks="1 month")
...
caselabdata <- casedata %>% inner_join(labdata, by=c('date')) %>%
  mutate(positive_rate7 = positive_cases7 / daily_tests7)
