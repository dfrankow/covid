...
```{r per_county_graph}
rate_per_county_graph <- function(num_panels=2) {
  wcr_by_county <- read_csv('data/wcrmap.csv') %>%
...
#           value1 = cut(value, c(0,5,10,15,25,50,75,100,Inf)),
  ...
  plot <- ggplot(wcr_by_county,
                 aes(week1a, County, fill=value1)) + geom_tile() +
    scale_y_discrete(limits = rev) +
    scale_x_discrete(position="top") +
    labs(x=NULL, y=NULL,
         subtitle="Weekly case rate per 10,000 people by county") +
...
```{r case_rate_by_county, fig.height=5}
rate_per_county_graph()
```
