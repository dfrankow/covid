...
vaccine_plot <- function() {
  library(lubridate)
  vax <- read_tsv('data/adminvax.tsv') %>%
    rename(week=`MMWR Week`,
           num_administered=`Number of vaccines  administered`,
           total_administered=`Total number of vaccines  administered`
           ) %>%
    separate(week, into=c('week', 'dates'), sep=": ") %>%
    separate(dates, into=c('start_date', 'end_date'), sep='-') %>%
    mutate(
      the_year = ifelse(row_number() <= 3, "20", "21"),
      start_date = parse_date(
        ifelse(str_count(start_date, "/")==1,
               paste0(start_date,"/", the_year), start_date),
        "%m/%d/%y"),
      end_date = parse_date(
        ifelse(str_count(end_date, "/")==1,
               paste0(end_date,"/", the_year), end_date),
        "%m/%d/%y")
    )
  vax_this_week <- vax %>% filter(!is.na(start_date)) %>% tail(1)
  ggplot(vax, aes(start_date, num_administered)) +
    geom_bar(stat="identity") +
    scale_x_date(
      breaks=vax$start_date,
      labels=function(breaks) {
        vax %>% inner_join(data.frame(start_date=breaks)) %>%
          mutate(the_label=paste0(
            format(start_date, "%b %e"),
            " - ",
            format(end_date, "%b %e")),
    ...
    labs(x="Week (so far)", y="# vaccines administered",
         title="Vaccines administered",
         subtitle=paste0(
           vax_this_week$num_administered, " this week, ",
...
deathdata <- deathdata %>%
  mutate(
    the_year = ifelse(row_number() <= 286, "2020", "2021"),
    date = as.Date(paste0(the_year, "/", date), format="%Y/%m/%d"),
    gt_prev_date = date > lag(date),
    # july 4 had "-" in the data
