...
casedata <- casedata %>%
  ...
  add_rolling_num('confirmed_cases', 7)
...
ggplot(casedataf,
       aes(date, confirmed_cases)) +
  geom_point(aes(alpha=alpha)) +
  geom_line(aes(alpha=alpha)) +
  geom_line(data=casedataf1, aes(y=confirmed_cases7), size=1) +
  labs(title="Daily confirmed cases",
       subtitle=paste0(
         "average ", round(tail(casedataf1$confirmed_cases7, 1)),
         ", 14-day change ",
         change_in_percent(casedataf1$confirmed_cases7, CHANGE_WINDOW), ".",
         "  Last 7 days may have incomplete data.")) +
  scale_y_continuous(limits=c(0,NA)) +
  scale_alpha_identity() +
  scale_x_date(date_minor_breaks="1 month")
...
labdata <- labdata %>% mutate(
  ...
  daily_tests_pcr = total_cumulative_tests - lag(total_cumulative_tests)
) %>%
  add_rolling_num('daily_tests_pcr', 7)
...
ggplot(labdataf, aes(date, daily_tests_pcr)) +
  geom_point(aes(alpha=alpha)) +
  geom_line(data=labdataf, aes(alpha=alpha)) +
  geom_line(data=labdataf1, aes(y=daily_tests_pcr7), size=1) +
  labs(title="Daily PCR tests",
       subtitle=paste0(
         "average ", round(tail(labdataf1$daily_tests_pcr7, 1)),
         ", 14-day change ",
         change_in_percent(labdataf1$daily_tests_pcr7, CHANGE_WINDOW), ".",
         "  Last 7 days may have incomplete data.")) +
  scale_y_continuous(limits=c(0,NA)) +
  scale_alpha_identity() +
  scale_x_date(date_minor_breaks="1 month")
...
caselabdata <- casedata %>% inner_join(labdata, by=c('date')) %>%
  mutate(positive_rate7 = confirmed_cases7 / daily_tests_pcr7)
