...
peak_list <- function(rolling_vec, date_vec) {
  ...
  percent_peak_rolling <- round(100 * tail(rolling_vec, 1) / max_rolling, 0)
  return(list(
...
daily_tests_peak <- peak_list(labdataf$daily_tests_rolling, labdataf1$date)
...
- Daily cases: currently at `r positive_cases_peak$percent_peak_rolling`% of peak (`r round(positive_cases_peak$max_rolling, 0)` people,  `r positive_cases_peak$max_rolling_days` days ago)

- Daily tests: currently at `r daily_tests_peak$percent_peak_rolling`% of peak (`r round(daily_tests_peak$max_rolling, 0)` people,  `r daily_tests_peak$max_rolling_days` days ago)

- Positive test rate: currently at `r positive_test_rate_peak$percent_peak_rolling`% of peak (`r round(positive_test_rate_peak$max_rolling, 0)` people,  `r positive_test_rate_peak$max_rolling_days` days ago)
